...
# 固定读取 backend/.env（相对于当前文件的位置）
BACKEND_DIR = Path(__file__).resolve().parent.parent
BACKEND_ENV_FILE = BACKEND_DIR / ".env"


class Settings(BaseSettings):
    """应用配置，从环境变量和 backend/.env 文件加载"""
    ...
    llm_provider: str = Field(
        default="claude",
        description="LLM Provider: claude, synthetic, qwen-portal, qwen, gemini, ollama, opencode, siliconflow",
    )
    llm_fallback: str = Field(
        default="",
        description="备用 Provider 列表（逗号分隔，按优先级），如: qwen,ollama；留空则不启用 fallback",
    )
    ...
    def get_anthropic_api_key(self) -> Optional[str]:
        ...
        from backend.providers.auth import get_api_key_from_credentials
    ...
    siliconflow_base_url: str = Field(
        ...
        description="SiliconFlow OpenAI 兼容 API Base URL",
    )
    siliconflow_model: str = Field(
        default="Qwen/Qwen2.5-7B-Instruct",
        description="SiliconFlow 对话模型名称（示例值，可按平台支持情况调整）",
    )
    siliconflow_embedding_model: str = Field(
        default="BAAI/bge-large-zh-v1.5",
        description="SiliconFlow 向量模型名称（示例值，可按平台支持情况调整）",
    )
